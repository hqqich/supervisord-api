...
import me.hqqich.supervisord.exceptions.SupervisordException;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public class Supervisord {

    private static final Logger LOGGER = LoggerFactory.getLogger(Supervisord.class);

    private static final String DEFAULT_URL = "http://172.22.90.3:9001/RPC2";
    private static final String DEFAULT_NAMESPACE = "supervisor";
    ...
    /**
     * this can be got by the api getIdentification(#link getIdentification()); but to save one http request, better to set it static
     *
    ...
    /**
     * Send a string of chars to the stdin of the process name. If non-7-bit data is sent (unicode), it is encoded to utf-8 before being sent to the process’ stdin. If chars is not a string or is not unicode, raise INCORRECT_PARAMETERS. If the process is not running, raise NOT_RUNNING. If the
     * process’ stdin cannot accept input (e.g. it was closed by the child process), raise NO_FILE.
     *
     ...
     * <p>
     * Requests (length) bytes from the (name)’s log, starting at (offset). If the total log size is greater than (offset + length), the overflow flag is set and the (offset) is automatically increased to position the buffer at the end of the log. If less than (length) bytes are available, the
     * maximum number of available bytes will be returned. (offset) returned is always the last offset in the log +1.
     *
     * @param name   the name of the process (or ‘group:name’)
     * @param offset offset to start reading from
